...
if (process.platform !== "darwin" || process.arch !== "arm64") {
  throw new Error("Stem performance qualification requires macOS on Apple Silicon");
}
const python = join(root, "src-tauri/resources/python-runtime/runtime/bin/python3.13");
const ffmpeg = join(root, "src-tauri/resources/audio-tools/bin/ffmpeg");
...
try {
  ...
  const argumentsList = ["-m", "sonarcan_mlx_worker", "separate", "--input", input, "--output", output, "--model-dir", model, "--ffmpeg", ffmpeg, "--profile", "hq"];

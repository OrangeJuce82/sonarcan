<script lang="ts">
  ...
  let { title, close, children, wide = false, keydown }: { title: string; close: () => void; children: import("svelte").Snippet; wide?: boolean; keydown?: (event: KeyboardEvent) => void } = $props();
